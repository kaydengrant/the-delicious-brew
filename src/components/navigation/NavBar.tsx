...
const NavBar: React.FC = () => {
  ...
  return (
    <>
      {(shopOpen || blogOpen || hamburgerOpen) && (
        <div
          className="absolute inset-0 opacity-90 z-10 bg-black "
          onClick={() => {
            setHamburgerOpen(false);
          }}
        />
      )}
      <header className="flex flex-col items-center w-screen header-container">
        <nav
          onMouseLeave={() => {
            setShowSearchBar(false);
          }}
          className={`flex flex-row w-full justify-center bg-white`}
        >
          <div
        ...
        {hamburgerOpen && (
          <>
            ...
            />
            <NavMobile status={setHamburgerOpen} />
          </>
        )}
      </header>
    </>
